...
var roleStorageMover = require('roleStorageMover');
    ...
    profiler.wrap(function() {
        ...
        */
        ...
        for (var i in Memory.WorldManager.OperateRooms) {
            ...
            var storageMovers = _.filter(Game.creeps, (creep) => creep.memory.role == 'storageMover' && creep.memory.homeRoom == i);
            ...
            var containterIDs = [];
            for (var y in movers) {
                var containterID = movers[y].memory.assignedContainer;
                containterIDs.push(containterID);
            }

            var containers = Game.rooms[i].find(FIND_STRUCTURES, {
            ...
            for (var x in roomInfo.Spawns) {
                ...
                for(var containter in containers){
                  var containterID = containers[containter].id;
                  console.log('Need Some Movers: '+  containterID);
                  var spot = containterIDs.indexOf(containterID);
                  if (spot == -1) {
                      if (movers.length == 0) {
                          name = spawn.createMover(maxEnergeyAvailable, "mover");
                      } else {
                          name = spawn.createMover(maxEnergeyAvailable, "mover");
                      }
                      ...
                      if (!(name < 0)) {
                          ...
                          newCreep.memory.homeSpawn = spawn.id;
                          newCreep.memory.homeRoom = roomName;
                          newCreep.memory.assignedContainer = containterID;
                      }
                  }
                }

                if (storageMovers.length < NUM_STORAGE_MOVERS) {
                    var newName = spawn.createMover(maxEnergeyAvailable, "storageMover");
                    if (movers.length <2) {
                    var newName = spawn.createMover(currentEnergyAvailable, "storageMover");
                    }
                }
                ...
                if (builders.length < NUM_BUILDER) {
                    ...
                    var newName = spawn.createCreep([WORK, CARRY, CARRY,  MOVE], undefined, {
                        role: 'repairer',
                        repairing: false,
        ...
        for (var name in Game.creeps) {
            ...
            if (creep.memory.role == 'harvester') {
                ...
                roleMover.run(creep)
            } else if (creep.memory.role == 'storageMover') {
                roleStorageMover.run(creep)
